...
const SCHICHTPLAN_FIELDS = [
  ...
  { key: 'mitarbeiter_fruehschicht', label: 'Mitarbeiter Frühschicht', type: 'applookup/select', targetEntity: 'mitarbeitende', targetAppId: 'MITARBEITENDE', displayField: 'vorname' },
  { key: 'mitarbeiter_spaetschicht', label: 'Mitarbeiter Spätschicht', type: 'applookup/select', targetEntity: 'mitarbeitende', targetAppId: 'MITARBEITENDE', displayField: 'vorname' },
  { key: 'notiz', label: 'Notiz zum Tag', type: 'string/textarea' },
];
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('schichtplan');
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'schichtplan' && fieldKey === 'mitarbeiter_fruehschicht') {
      const match = (lists.mitarbeitendeList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.vorname ?? '—';
    }
    if (entity === 'schichtplan' && fieldKey === 'mitarbeiter_spaetschicht') {
      const match = (lists.mitarbeitendeList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.vorname ?? '—';
    }
    return String(url);
